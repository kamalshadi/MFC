...
class Node(object):
    ...
    def __init__(self, data=None, left=None, right=None, value = None):
        self.data = data
        self.left = left
        self.right = right
        self.value = value
...
class KDNode(Node):
    ...
    def __init__(self, data=None, left=None, right=None, axis=None,
        ...
        node. """
        super(KDNode, self).__init__(data, left, right, value)
        self.axis = axis
    ...
    def add(self, point):
        ...
        while True:
            ...
            if current.data is None:
                current.data = point.centroid
                current.value = point
                return current

            # split on self.axis, recurse either left or right
            if point.centroid[current.axis] < current.data[current.axis]:
    ...
    @require_axis
    def create_subnode(self, point):
        """ Creates a subnode for the current node """

        return self.__class__(point.centroid,
                axis=self.sel_axis(self.axis),
                sel_axis=self.sel_axis, value = point)
...
def create(point_list=None, axis=0, sel_axis=None):
    ...
    #~ if not point_list:
        #~ raise ValueError('point_list must be provided')


    # by default cycle through the axis
    sel_axis = sel_axis or (lambda prev_axis: (prev_axis+1) % 2)
    if len(point_list) == 0:
        return KDNode(sel_axis=sel_axis, axis=axis)
    if len(point_list) == 1:
		return KDNode(point_list.keys()[0], None, None, axis=axis, sel_axis=sel_axis,value = point_list.values()[0])
		
    # Sort point list and choose median as pivot element
    pl= point_list.keys()
    pl.sort(key=lambda point: point[axis])
    median = len(pl) // 2

    loc   = pl[median]
    
    left  = create({key:point_list[key] for key in pl[:median]},sel_axis(axis))
    right = create({key:point_list[key] for key in pl[median + 1:]},sel_axis(axis))
    return KDNode(loc, left, right, axis=axis, sel_axis=sel_axis,value = point_list[loc])
...
if __name__ == '__main__':
	a = {(1,4):2,(3,2):4,(5,1):34,(1,3):0}
	t = create(a)
	A = t.search_nn_dist((2,2),11)
	print(A)
	for w in A:
		print(w.data)
		print(w.value)
